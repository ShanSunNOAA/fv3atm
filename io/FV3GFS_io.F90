...
!-----------------------------------------------------------------------
  ...
  character(len=32)  :: fn_emi12m = 'emi12m_data.nc'
  ...
  type(restart_file_type) :: Oro_restart, Sfc_restart, Phy_restart, dust_restart, dust12m_restart, emi_restart, emi12m_restart, emi2_restart, gbbepx2_restart, gbbepx_restart, modis_restart
  ...
  character(len=32),    allocatable,         dimension(:)       :: dust_name, dust12m_name, emi_name, emi12m_name, emi2_name, gbbepx2_name, gbbepx_name, modis_name
  ...
  real(kind=kind_phys), allocatable, target, dimension(:,:,:,:) :: dust12m_var, emi12m_var
...
!----------------------------------------------------------------------      
  subroutine sfc_prop_restart_read (Sfcprop, Atm_block, Model, fv_domain, warm_start)
    ...
    integer :: nvar_dust12m, nvar_emi12m
...
! print diag info
...
#ifdef EMI12m_DATA
    if (.not. allocated(emi12m_name)) then
    !--- allocate the various containers needed for orography data
      allocate(emi12m_name(nvar_emi12m))
      allocate(emi12m_var(nx,ny,12,nvar_emi12m))

      emi12m_name(1)  = 'e_bc'
      emi12m_name(2)  = 'e_oc'
      emi12m_name(3)  = 'e_sulf'
      emi12m_name(4)  = 'e_pm_25'
      emi12m_name(5)  = 'e_so2'
      emi12m_name(6)  = 'e_pm_10'
      emi12m_name(7)  = 'dm0'
      emi12m_name(8)  = 'ero1'
      emi12m_name(9)  = 'ero2'
      emi12m_name(10) = 'ero3'
      !--- register the 2D fields
      do num = 1,nvar_emi12m
        var3_p2 => emi12m_var(:,:,:,num)
        id_restart = register_restart_field(emi12m_restart, fn_emi12m, emi12m_name(num), var3_p2, domain=fv_domain,mandatory=mand)
      enddo
      nullify(var3_p2)
    endif

    !--- read new GSD created emi12m restart/data
    call mpp_error(NOTE,'reading emi12m information from INPUT/emi12m_data.tile*.nc')
    call restore_state(emi12m_restart)
    ...
    do nb = 1, Atm_block%nblks
      ...
      do ix = 1, Atm_block%blksz(nb)
        ...
        !--- assign hprime(1:10) and hprime(15:24) with new oro stat data
        do k = 1, 12
          Sfcprop(nb)%emi12m_in(ix,k,1)  = emi12m_var(i,j,k,1)
          Sfcprop(nb)%emi12m_in(ix,k,2)  = emi12m_var(i,j,k,2)
          Sfcprop(nb)%emi12m_in(ix,k,3)  = emi12m_var(i,j,k,3)
          Sfcprop(nb)%emi12m_in(ix,k,4)  = emi12m_var(i,j,k,4)
          Sfcprop(nb)%emi12m_in(ix,k,5)  = emi12m_var(i,j,k,5)
          Sfcprop(nb)%emi12m_in(ix,k,6)  = emi12m_var(i,j,k,6)
          Sfcprop(nb)%emi12m_in(ix,k,7)  = emi12m_var(i,j,k,7)
          Sfcprop(nb)%emi12m_in(ix,k,8)  = emi12m_var(i,j,k,8)
          Sfcprop(nb)%emi12m_in(ix,k,9)  = emi12m_var(i,j,k,9)
          Sfcprop(nb)%emi12m_in(ix,k,10) = emi12m_var(i,j,k,10)
        enddo
      enddo
    enddo

    call free_restart_type(emi12m_restart)
#endif
    ...
    !--- allocate the various containers needed for orography data
      ...
      allocate(gbbepx2_var(nx,ny,60,nvar_gbbepx2))

      gbbepx2_name(1)  = 'ebu_bc'
      gbbepx2_name(2)  = 'ebu_oc'
      gbbepx2_name(3)  = 'ebu_pm_25'
      gbbepx2_name(4)  = 'ebu_so2'
      gbbepx2_name(5)  = 'ebu_frp'
      !--- register the 3D fields
      mand = .false.
      do num = 1,nvar_gbbepx2
        var3_p2 => gbbepx2_var(:,:,:,num)
        id_restart = register_restart_field(gbbepx2_restart, fn_gbbepx2, gbbepx2_name(num), var3_p2, domain=fv_domain,mandatory=mand)
      enddo
      nullify(var3_p2)
    endif

    !--- read new GSD created gbbepx2 restart/data
    call mpp_error(NOTE,'reading gbbepx2 information from INPUT/gbbepx2_data.tile*.nc')
    call restore_state(gbbepx2_restart)
    ...
    do nb = 1, Atm_block%nblks
      ...
      do ix = 1, Atm_block%blksz(nb)
        ...
        do k = 1, 60
          ...
          Sfcprop(nb)%fire2_GBBEPx(ix,k,4)  = gbbepx2_var(i,j,k,4)
          Sfcprop(nb)%fire2_GBBEPx(ix,k,5)  = gbbepx2_var(i,j,k,5)
        enddo
      enddo
    enddo

    call free_restart_type(gbbepx2_restart)
#endif
