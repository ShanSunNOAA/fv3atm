...
!!      enddo
   ...
   if (Model%me == Model%master) then
     print*, 'FIRE2 Sfcprop array diag: fire2_GBBEPx'
     do k = 1, 5
       print*, 'Day', k
       do nb = 1, Atm_block%nblks
          print*, 'Block', nb
          print*, Sfcprop(nb)%fire2_GBBEPx(:,k,5)
       enddo
     enddo
    endif

    call free_restart_type(gbbepx2_restart)
    deallocate(gbbepx2_name, gbbepx2_var)
#endif
...
!       if(Model%frac_grid) then
          ...
          Sfcprop(nb)%tsfc(ix)   = sfc_var2(i,j,36) !--- tsfc composite
          Sfcprop(nb)%zorl(ix)   = sfc_var2(i,j,37) !--- zorl composite
!       else
